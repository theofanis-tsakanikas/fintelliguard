...
**Two clouds, on purpose — and you could do it in one.** All three tiers *could* run on Databricks alone (Mosaic AI has model serving, an agent framework, and vector search), so two clouds is a deliberate choice, not a technical necessity. It models a real brownfield bank: the real-time edge lives in AWS, near the payment gateway; the lakehouse lives in Databricks — two teams joined by a contract, not coupling. Bedrock owns the real-time *regulated* verdict at the edge, with managed Guardrails + Knowledge Bases; Databricks owns the async copilot where the data lives; Tier 1 trains and serves where the features live (feature parity). The two meet only through `get_fraud_score()` over a private endpoint, so the external agent never reads Delta or raw data — a trust boundary you lose if everything sits in one platform. A greenfield build with no AWS footprint would be simpler in one place (one bill, one IAM); this chose realism, separation, and managed compliance at the cost of the cross-cloud integration it also exists to demonstrate.
